...
import { Point, Spring } from "./model.js";
...
function chainv() {
    ...
    {
        Points[0].updatePosition = (t) => { Points[0].r.y = Math.sin(t * 5) * 100; };
...
function chainl() {
    ...
    Points = new Array(num).fill(0).map((x, i) => new Point(new Vec3(i * 10, 1, 0), mass));
    {
        Points[0].updatePosition = (t) => { Points[0].r.x = Math.sin(t * 5) * 100; };
        Points[num - 1].m = 10000000000;
    }
    Springs = new Array(num - 1).fill(0).map((x, i) => new Spring(k, l, Points[i], Points[i + 1]));
    step = 0.001;
...
function chainc() {
    ...
    Points = new Array(num).fill(0).map((x, i) => new Point(new Vec3(i * 10, 1, 0), mass));
    {
        Points[0].updatePosition = (t) => { Points[0].r.z = Math.cos(t * 5) * 100; Points[0].r.y = Math.sin(t * 5) * 100; };
        Points[num - 1].m = 10000000000;
    }
    Springs = new Array(num - 1).fill(0).map((x, i) => new Spring(k, l, Points[i], Points[i + 1]));
    step = 0.001;
...
function net() {
    ...
    Points[1 + 1 * numx].updatePosition = (t) => { Points[1 + 1 * numx].r.z = Math.sin(t * 2) * 500; };
...
var speed = 1;
var t = 0;
var before = Number(new Date()) + 1000;
function loop() {
    var dt = Number(new Date()) - before;
